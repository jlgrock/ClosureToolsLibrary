	...
	Parsed parseFile(final File fileToParse) {
		...
		try {
			...
			testCase = new Parsed(fileToParse,
					TestResultType.TIMED_OUT, te.message)
			encounteredError = true
		} catch (NotFoundException nfe) {
			testCase = new Parsed(fileToParse,
					TestResultType.BAD_OUTPUT, nfe.message)
			encounteredError = true
		} catch (WebDriverException se) {
			testCase = new Parsed(fileToParse,
					TestResultType.SCRIPT_ERROR, se.message)
			encounteredError = true
			...
			testCase = new Parsed(fileToParse,
					TestResultType.SCRIPT_ERROR, mue.message)
			encounteredError = true
		} catch (Exception e) {
			testCase = new Parsed(fileToParse,
					TestResultType.UNABLE_TO_EXECUTE, e.message)
			encounteredError = true
		}
